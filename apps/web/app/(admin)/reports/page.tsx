...
export default function ReportsPage() {
  ...
  const [profit, setProfit] = useState<{ revenue: string; cogs: string; refunds: string; expenses: string; netProfit: string } | null>(null);
  const [from, setFrom] = useState('');
  const [to, setTo] = useState('');
  const [shopId, setShopId] = useState('');
  const [shops, setShops] = useState<{ id: string; name: string }[]>([]);
  const query = `?${new URLSearchParams({ ...(from ? { from } : {}), ...(to ? { to } : {}), ...(shopId ? { shopId } : {}) }).toString()}`;
  useEffect(() => {
    api<{ id: string; name: string }[]>('/shops').then((r) => setShops(r.data ?? [])).catch(() => undefined);
  }, []);
  useEffect(() => {
    api<NonNullable<typeof data>>(`/reports/sales${query}`).then((r) => setData(r.data)).catch(console.error);
    api<NonNullable<typeof dimensions>>(`/reports/sales/dimensions${query}`).then((r) => setDimensions(r.data)).catch(() => undefined);
    api<NonNullable<typeof inventory>>(`/reports/inventory/summary${shopId ? `?shopId=${encodeURIComponent(shopId)}` : ''}`).then((r) => setInventory(r.data)).catch(() => undefined);
    api<NonNullable<typeof profit>>(`/reports/profit-loss${query}`).then((r) => setProfit(r.data)).catch(() => undefined);
  }, [from, to, shopId]);

  async function download(format: 'csv' | 'pdf' | 'xlsx') {
    const result = await api<{ filename: string; mimeType: string; contentBase64: string }>(`/reports/sales/export?format=${format}${from ? `&from=${encodeURIComponent(from)}` : ''}${to ? `&to=${encodeURIComponent(to)}` : ''}${shopId ? `&shopId=${encodeURIComponent(shopId)}` : ''}`);
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 rounded-xl bg-white p-4 shadow-sm"><input type="date" className="rounded-lg border px-3 py-2 text-sm" value={from} onChange={(e) => setFrom(e.target.value)} /><input type="date" className="rounded-lg border px-3 py-2 text-sm" value={to} onChange={(e) => setTo(e.target.value)} /><select className="rounded-lg border px-3 py-2 text-sm" value={shopId} onChange={(e) => setShopId(e.target.value)}><option value="">All shops</option>{shops.map((shop) => <option key={shop.id} value={shop.id}>{shop.name}</option>)}</select></div>
      {data ? (
      ...
      {profit ? <section className="mt-6 grid gap-4 sm:grid-cols-5">{Object.entries({ Revenue: profit.revenue, COGS: profit.cogs, Refunds: profit.refunds, Expenses: profit.expenses, 'Net profit': profit.netProfit }).map(([label, value]) => <div key={label} className="rounded-xl bg-white p-4 shadow-sm"><span className="text-xs text-slate-500">{label}</span><strong className="mt-1 block text-xl">{value}</strong></div>)}</section> : null}
